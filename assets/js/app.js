  ...
  initTopbarMenu() {
    $(".navbar-nav").length &&
      ($(".navbar-nav li a").each(function () {
        var e = window.location.href.split(/[?#]/)[0];
        this.href == e &&
          ($(this).addClass("active"), $(this).parent().parent().addClass("active"), $(this).parent().parent().parent().parent().addClass("active"), $(this).parent().parent().parent().parent().parent().parent().addClass("active"));
      }),
  ...
  initTopbarScroll() {
    ...
    if (a) {
      ...
      window.addEventListener("scroll", function () {
        ...
        30 <= e && !t ? (a.classList.add("topbar-active"), (t = !0)) : e < 30 && t && (a.classList.remove("topbar-active"), (t = !1));
      });
  ...
  initSwitchListener() {
    ...
    e &&
      ...
      document.querySelectorAll(".button-sm-hover").forEach(function (e) {
        e.addEventListener("click", function () {
          var e = a.config.sidenav.size;
          "sm-hover-active" === a.html.getAttribute("data-sidenav-size", e) ? a.changeLeftbarSize("sm-hover", !0) : a.changeLeftbarSize("sm-hover-active", !0);
        });
      });
  }
  showBackdrop() {
...
document.addEventListener("DOMContentLoaded", function () {
  const btn = document.getElementById("dropdownTahun");
  document.querySelectorAll("#dropdownTahun ~ .dropdown-menu .dropdown-item").forEach((item) => {
    item.addEventListener("click", function (e) {
      e.preventDefault();
      const selected = this.getAttribute("data-value");
      btn.textContent = selected; // ubah teks tombol sesuai pilihan
      console.log("Tahun dipilih:", selected);
      // Di sini bisa dipanggil fungsi update chart pakai tahun terpilih
    });
  });
});
//end

//Loading Halamanan dan Aksi
...
function showToast(message, type = "info") {
  ...
  const toastHTML = `
    <div class="toast align-items-start custom-toast" role="alert" aria-live="assertive" aria-atomic="true">
...
// === Loading saat melakukan aksi ===
document.getElementById("btnAction").addEventListener("click", () => {
  loading.style.display = "flex";

  // simulasi proses sukses/gagal random
  setTimeout(() => {
    loading.style.display = "none";
    if (Math.random() > 0.5) {
...
function checkChanges() {
  ...
  inputs.forEach((input, i) => {
    if (input.value !== initialValues[i]) {
      changed = true;
    }
  });
...
// Event listener untuk input teks & password
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  navLinks.forEach((link) => {
    ...
    // Bandingkan berdasarkan awalan nama file (bukan exact match)
    // contoh: apps-certificate.html, apps-certificate-create.html, apps-certificate-edit.html
    const baseHref = href.split(".")[0]; // ambil "apps-certificate"
    const basePath = currentPath.split(".")[0]; // ambil "apps-certificate-create"

    if (basePath.startsWith(baseHref)) {
      // tandai link ini sebagai aktif
      link.classList.add("active");
      const parentItem = link.closest(".side-nav-item");
      if (parentItem) parentItem.classList.add("active");

      // jika di dalam sub-menu, buka collapsenya
